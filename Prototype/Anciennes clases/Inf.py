...
class Inf(QWidget):
    ...
    def __init__(self, largeur, hauteur, viewer, parent=None):
        super(Inf, self).__init__(parent=parent)
        self.parent = parent
        self.viewer = viewer
    ...
    def creationInterfaceBat(self):
        ...
        listeNom = ["Niveau du quartier", "Caserne", "Camps d'entrainement", "Baraquement des fantassins"]
        for i in range(0, len(listeNom)):
            self.listeBat.append(LigneBat(listeNom[i], i, self.widgetBat, "Inf", self))
    ...
    def majBat(self):
        ...
        nvQuart = self.ville.batInf[0]
        for i in range(1, len(self.ville.batInf)):
            self.ville.bdd.execute("""SELECT nvQuart FROM Ville WHERE Quartier == "Inf" AND Batiment == %s AND Niveau == 1""" % (i))
    ...
    def setVille(self, ville):
        "Methode permettant de mettre a jour le widget Inf en fonction de la ville"
        self.ville = ville
