...
class Cav(QWidget):
    ...
    def __init__(self, largeur, hauteur, viewer, parent=None):
        super(Cav, self).__init__(parent=parent)
        self.parent = parent
        self.viewer = viewer
    ...
    def creationInterfaceBat(self):
        ...
        listeNom = ["Niveau du quartier", "Ecurie", "Champs de manaoeuvres", "Atelier des chars", "Ecole militaire"]
        for i in range(0, len(listeNom)):
            self.listeBat.append(LigneBat(listeNom[i], i, self.widgetBat, "Cav", self))
    ...
    def majBat(self):
        ...
        nvQuart = self.ville.batCav[0]
        for i in range(1, len(self.ville.batCav)):
            self.ville.bdd.execute("""SELECT nvQuart FROM Ville WHERE Quartier == "Cav" AND Batiment == %s AND Niveau == 1""" % (i))
    ...
    def setVille(self, ville):
        "Methode permettant de mettre a jour le widget Cav en fonction de la ville"
        self.ville = ville
