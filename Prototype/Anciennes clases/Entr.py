...
class Entr(QWidget):
    ...
    def __init__(self, largeur, hauteur, viewer, parent=None):
        super(Entr, self).__init__(parent=parent)
        self.parent = parent
        self.viewer = viewer
    ...
    def creationInterfaceBat(self):
        ...
        listeNom = ["Niveau du quartier", "Entrepôt de bois", "Entrepôt de pierre", "Entrepôt de fer", "Entrepôt d'or", "Entrepôt de charbon", "Entrepôt de marbre", "Entrepôt de nourriture", "Entrepôt d'acier", "Fonderie"]
        for i in range(0, len(listeNom)):
            self.listeBat.append(LigneBat(listeNom[i], i, self.widgetBat, "Entr", self))
    ...
    def majBat(self):
        ...
        nvQuart = self.ville.batEntr[0]
        for i in range(1, len(self.ville.batEntr)):
            self.ville.bdd.execute("""SELECT nvQuart FROM Ville WHERE Quartier == "Entr" AND Batiment == %s AND Niveau == 1""" % (i))
    ...
    def show(self):
        ...
        super(Entr, self).show()
        self.afficheBat()

    def setVille(self, ville):
        "Methode permettant de mettre a jour le widget Entr en fonction de la ville"
        self.ville = ville
