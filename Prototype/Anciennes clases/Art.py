...
class Art(QWidget):
    ...
    def __init__(self, largeur, hauteur, viewer, parent=None):
        super(Art, self).__init__(parent=parent)
        self.parent = parent
        self.viewer = viewer
    ...
    def creationInterfaceBat(self):
        ...
        listeNom = ["Niveau du quartier", "Camp de tir", "Ecurie", "Atelier"]
        for i in range(0, len(listeNom)):
            self.listeBat.append(LigneBat(listeNom[i], i, self.widgetBat, "Art", self))
    ...
    def majBat(self):
        ...
        nvQuart = self.ville.batArt[0]
        for i in range(1, len(self.ville.batArt)):
            self.ville.bdd.execute("""SELECT nvQuart FROM Ville WHERE Quartier == "Art" AND Batiment == %s AND Niveau == 1""" % (i))
    ...
    def setVille(self, ville):
        "Methode permettant de mettre a jour le widget Art en fonction de la ville"
        self.ville = ville
