...
class March(QWidget):
    ...
    def __init__(self, largeur, hauteur, viewer, parent=None):
        super(March, self).__init__(parent=parent)
        self.parent = parent
        self.viewer = viewer
    ...
    def creationInterfaceBat(self):
        ...
        listeNom = ["Niveau du quartier", "Marché", "Comptoir commercial", "Bâtiments des Guildes"]
        for i in range(0, len(listeNom)):
            self.listeBat.append(LigneBat(listeNom[i], i, self.widgetBat, "March", self))
    ...
    def majBat(self):
        ...
        nvQuart = self.ville.batMarch[0]
        for i in range(1, len(self.ville.batMarch)):
            self.ville.bdd.execute("""SELECT nvQuart FROM Ville WHERE Quartier == "March" AND Batiment == %s AND Niveau == 1""" % (i))
    ...
    def show(self):
        ...
        super(March, self).show()
        self.afficheBat()

    def setVille(self, ville):
        "Methode permettant de mettre a jour le widget March en fonction de la ville"
        self.ville = ville
